...
mle_model = HeteroNetMLE(hidden=16)
optimizer = torch.optim.Adam(mle_model.parameters(), lr=3e-3, weight_decay=5e-4)

t0 = time.time()
for step in range(10000):
    optimizer.zero_grad()
    mu, sigma = mle_model(X)
    loss = -dist.Normal(mu, sigma).log_prob(y).mean()
    loss.backward()
    torch.nn.utils.clip_grad_norm_(mle_model.parameters(), max_norm=5.0)
    optimizer.step()
t_mle = time.time() - t0

with torch.no_grad():
    mu_g, sigma_g = mle_model(Xg)
    y_mle_mean = mu_g.numpy()
    y_mle_lo = (mu_g - 1.96 * sigma_g).numpy()
    y_mle_hi = (mu_g + 1.96 * sigma_g).numpy()
    mse_mle = float(np.mean((y_mle_mean - y_true_g.numpy())**2))
    
    mu_train, sigma_train = mle_model(X)
    lo_train = mu_train - 1.96 * sigma_train
    hi_train = mu_train + 1.96 * sigma_train
    cov_mle = float(torch.mean(((y >= lo_train) & (y <= hi_train)).float()))
    width_mle = float(torch.mean(hi_train - lo_train))
################
# -----------------------------
